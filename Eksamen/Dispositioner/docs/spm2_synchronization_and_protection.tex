...
Tråden som har mutexen gives højeste prioritet (priority ceiling). Denne prioritet assignes til ejeren af mutexen så snart den tages. Se figur~\ref{fig:ceili}.
...
\begin{lstlisting}[otherkeywords={un, item, buffer, unlock, lock, signal}]
...
\begin{enumerate}
	\item \textbf{Mutual exclution} - \textit{at least one resource must be held in a non-shareable mode}.\\
	Nope, de skal have hver deres egen gaffel.
	\item \textbf{Hold and Wait} eller \textbf{Resource holding} - \textit{a process is currently holding at least one resource and requesting additional resources which are being held by other processes}.\\
	Heller ikke, de skal have to gafler for at spise.
	\item \textbf{No preemption\footnote{Læs om preemption i section~\ref{sec:execpu}}} - \textit{a resource can be released only voluntarily by the process holding it}.\\
	Nix, de må ikke tage hinandens gafler.
	\item \textbf{Circular wait} - \textit{a process must be waiting for a resource which is being held by another process, which in turn is waiting for the first process to release the resource}.\\
	Bingo! Vi kan bryde denne cyklus!
\end{enumerate}
...
\begin{enumerate}
	\item Filosofferne kan kun give en gaffel videre hvis den er beskidt. Alle gafler er beskidte fra start. 
	\item Filosofferne skal spørge sine naboer om lov når de ønsker en gaffel.
	\item Filosoffen beholder gaflen hvis den er ren.
	\item Filosofferne rengører gaflen lige før de sender den videre og efter de har modtaget et request.
	\item Når en filosof har spist er begge hans gafler beskidte.
