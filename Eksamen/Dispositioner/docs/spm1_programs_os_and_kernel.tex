...
\begin{itemize}
	\item Simpel implementering, ingen kernel support for threads.\todo{hvad bestød det nu igen?}
	\item Ekstremt hurtig thread kontekst skift ikke brug for kernel handling).\todo{kan ikke huske hvorfor?}
	\item Ikke muligt at håndtere flere CPU-core.\todo{igen hvorfor?}
\end{itemize}
...
\begin{itemize}
	\item OS kernel er bevidst om trådene. Giver overhead - er 100 gange langsommere end user level threads.\todo{skal der laves en kernel tråd hver gang vi skal bruge en user tråd?}
	\item Mapper direkte til "fysiske" threads som \textit{scheduleren} kan kontrollere.
	\item Effektiv brug af flere kerner.\todo{en kernel tråd = en fysisk tråd?}
\end{itemize}
...
\begin{itemize}
	\item Instruction cache - fremskynder instruction fetching.\todo{skal vi snakke om}
	\item Data cache - fremskynder fetch and store, og deles op i cache levels (L1, L2 osv.).
	\item TLB (Translation Lookaside Buffer) - fremskynder oversættelse af virtuel til fysiske addresser (TLB er en del af MMU'en\footnote{Memory Management Unit}).\todo{alt det fremskyden?}
\end{itemize}
